...
- **TASK-027:** UI-кнопки в видеоплеере скрываются через 3 сек во время воспроизведения, появляются по тапу или паузе. Добавлен `play`-event в JS-бридж (`videoPlaying` message); кнопка закрытия вынесена в отдельный всегда-видимый слой; прозрачный `Color.clear`-перехватчик заменён на `UITapGestureRecognizer` с `cancelsTouchesInView = false` — VK-плеер получает тапы одновременно с нашим обработчиком.
- **TASK-034:** pull-to-refresh добавлен в ленту (`.refreshable` на `ScrollView`); `loadFeed()` переделан в `@discardableResult Task` — индикатор висит до завершения загрузки. Кнопка «Обновить» в тулбаре и авто-скролл к `"feedTop"` работали уже раньше.
- **TASK-032:** сетка фото в посте — puzzle-раскладка. `case 3`: одно большое слева (высота `2h+4`) + два маленьких справа. Остальные раскладки (2, 4, 5, 6) уже были корректны.
- **Внеплановый фикс:** одиночные фото в ленте больше не обрезаются. Выделен `singlePhotoCell` с `.scaledToFit()` и `maxHeight: 520`; `gridCell` с `.fill` остался только для много-фото сеток.
...
- **TASK-006:** добавлено время к дате поста: сегодня — только `HH:mm`; текущий год — `d MMM в HH:mm`; прошлые годы — `d MMM yyyy в HH:mm`. `DateFormatter` с `locale = "ru_RU"`.
- **TASK-023:** исправлен свайп для переключения фото в fullscreen-галерее. Причина: `simultaneousGesture(DragGesture)` на контейнере `TabView` конкурировал с `UIScrollView` и блокировал горизонтальное листание. Решение: жест свайп-вниз (dismiss) перенесён внутрь каждой ячейки `FullScreenImageView` — там `simultaneousGesture` корректно сосуществует с UIScrollView родителя.
- **TASK-024:** (закрыт вместе с TASK-023) свайп вниз работает только при `scale == 1`; при `scale > 1` жест dismiss не вешается, изображение только панируется.

## Трек 2026-04-04 / 2026-04-05

- **TASK-001:** исправлена пагинация старых сообщений в чате. При подгрузке верхний message id сохраняется как anchor, после вставки старых сообщений список прокручивается обратно к прежней верхней точке, из-за чего индикатор "Загрузка старых сообщений" не зависает и подгрузка может срабатывать повторно.
- **TASK-002:** подтверждена рабочая загрузка аватара профиля через цепочку `PhotosPicker -> photos.getOwnerPhotoUploadServer -> upload -> photos.saveOwnerPhoto` с обновлением шапки профиля без перезапуска.
- **TASK-003:** исправлен и вручную проверен сценарий "Скачать на устройство" для fullscreen-фото. Сохранение идёт через временный файл и `PHAssetChangeRequest.creationRequestForAssetFromImage(atFileURL:)`, без прежнего краша.
- **TASK-004:** исправлено удаление фото из альбома. Корень бага — Swift ABI на ARM64: при вызове `@escaping async` замыкания с сигнатурой `(String, Int, Int)` оба Int-аргумента получали мусор (~0x103…/0x104…, адреса code-сегмента). String занимает 2 регистра (x0/x1), из-за чего Int-аргументы смещаются в регистры async-контекста. Решение: добавлен `GalleryDeleteRequest` (класс, ref-type); `AlbumPhotosView` держит его как `@State`, передаёт в галерею; `performDeleteCurrentPhoto` записывает `photoId` в объект синхронно до `await`, замыкание читает его из объекта минуя параметры.
