...
**Пул задач: fullscreen фото, посты, лента (закрыт)**
- **1. Zoom при просмотре фото:** FullScreenImageView — двойной тап (1x ↔ 2.5x), pinch (MagnificationGesture) 1x–4x, scaleEffect по изображению.
- **2. Свайп вниз для выхода:** заменён .gesture на .simultaneousGesture(DragGesture(minimumDistance: 30)) в FullScreenPhotoGalleryView, чтобы при нескольких фото в посте свайп вниз не перехватывался TabView. **Проверить на устройстве.**
- **3. Выравнивание постов по центру:** PostCellView — контейнер контента поста (postBodyContent) переведён с `.frame(maxWidth: .infinity, alignment: .leading)` на `.frame(maxWidth: .infinity, alignment: .center)`. Блок поста центрируется; текст внутри остаётся с выравниванием по ведущему краю.
- **4. Качество миниатюр в ленте:** VKPhoto.feedPreviewURL — приоритет размеров сменён с ["m", "x", ...] на ["x", "w", "z", "y", "m", ...], чтобы в ленте и репостах использовались более крупные размеры (x ≈ 604px и выше) при наличии.
- **5. Кнопка закрытия fullscreen:** в topBar галереи — xmark.circle.fill вместо chevron.down, размер 36 pt, область нажатия 56×56.
- **6. Автообновление ленты при заходе:** ContentView — при переходе приложения в активное состояние (scenePhase == .active) вызывается loadFeed(); при первом появлении таба «Лента» (onAppear) при состоянии .idle также вызывается loadFeed() для начальной загрузки после входа.
- **7. «Отписаться от паблика/группы»:** VKApiService.leaveGroup(token, groupId) — вызов groups.leave. GroupWallView: кнопка «Отписаться» в toolbar (рядом с «Обновить»); при успехе — dismiss(), при ошибке — alert с текстом.
